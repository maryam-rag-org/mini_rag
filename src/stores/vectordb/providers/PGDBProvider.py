...
class PGVectorProvider(VectorDBInterface):
    ...
    '''
        what we need ==> db client, vector size and the distance method
    '''
    ...
    def __init__(self, db_client, default_vector_size: int = 786,
        ...
        self.defalt_index_name = lambda collection_name: f"{collection_name}_vector_idx"
        ...
        async with self.db_client() as session:
                async with session.begin():
                    table_info_sql = sql_text(f'''
                                    ...
                                    FROM pg_tables 
                                    WHERE tablename LIKE :collection_name
                                        '''
                              )
                    ...
                    table_data = table_info.fetchone() # return tupel of (schemaname, tablename, tableowner, tablespace, hasindexes)
    ...
    async def is_index_existed(self, collection_name: str) -> bool:
        index_name = self.defalt_index_name(collection_name=collection_name)
    ...
    async def create_vector_index(self, collection_name: str, 
         ...
         async with self.db_client() as session:
                async with session.begin():
                     ...
                     index_name = self.defalt_index_name(collection_name)
    ...
    async def insert_one(self, collection_name: str,
        ...
        if not is_collection_existed:
            self.logger.info(f" collection: {collection_name} not existed")
            return False

        if not record_id: # = chunk_id
            self.logger.info(f"Can not insert recodr without  chunk id collection: {collection_name}")
            return False
        ...
        async with self.db_client() as session:
                async with session.begin():
                    insert_sql = sql_text(
                        ...
                        f'( {PgVectorTableSchemeEnums.TEXT.value}, {PgVectorTableSchemeEnums.VECTOR.value}, {PgVectorTableSchemeEnums.CHUNK_ID.value}, {PgVectorTableSchemeEnums.METADATA.value} ,{PgVectorTableSchemeEnums.CHUNK_ID.value} ) '
                    ...
                    await session.execute(insert_sql, {
                         "text": text,
                         "vector": "[" + ",".join([str(v) for v in vector]) + "]",  # pgvector take the vector as string
