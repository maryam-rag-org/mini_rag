...
class QwenProvider(LLMInterface):
    ...
    def process_text(self, text:str): # hekper methode that is why it is not in the interface
    ...
    def generate_text(self, prompt: str, chat_history: list=[],
        ...
        #raise NotImplementedError # For now We do not want to implement this function
        ...
        if not response or not response.choices or len(response.choices) == 0 or not response.choices[0].message:
            self.logger.error("Error while generating text with Qwen")
            return None
    ...
    def embed_text(self, text:str, document_type:str = None):
        ...
        if not response or not response.data or len(response.data) == 0 or not response.data[0].embedding:
            self.logger.error("Error while embedding text with Qwene")
            return None
